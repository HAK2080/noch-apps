...
// First, let's check what columns the recipes table has
...
// Try to get the column names through a different approach
async function getTableSchema() {
  try {
    console.log('Fetching table schema...');

    // This query uses Supabase's introspection
    ...
    if (error) {
      ...
      // Fallback: try inserting with minimal fields to see what's required
...
async function migrateRecipes() {
  try {
    ...
    // First, inspect what's actually in the table
    await inspectTable();
    await getTableSchema();

    console.log('\n📝 Attempting to insert recipes...\n');
    ...
    if (failCount > 0) {
      console.log('⚠️  Some recipes failed to insert.');
      console.log('This might be due to:');
      console.log('1. Missing table or columns in Supabase');
      console.log('2. Incorrect schema definition');
      console.log('3. RLS policies blocking the insert\n');
      console.log('💡 Solution: Go to Supabase Dashboard > SQL Editor and run:');
      console.log('   Copy the contents of supabase/schema_recipes.sql\n');
    }

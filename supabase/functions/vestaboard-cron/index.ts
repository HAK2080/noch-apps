...
function channelIsInWindow(channel: Channel, hour: number) {
  return channel.start_hour <= channel.end_hour
    ? hour >= channel.start_hour && hour < channel.end_hour
    : hour >= channel.start_hour || hour < channel.end_hour
}

async function getDueChannel(admin: ReturnType<typeof createClient>, now: Date) {
  const { data, error } = await admin
    .from('vestaboard_channels')
    .select('id,name,channel_type,cadence_minutes,start_hour,end_hour,config')
    .eq('enabled', true)
  ...
  return (data || []).find(channel => channelIsInWindow(channel as Channel, hour)) as Channel | undefined || null
}

async function buildChannelMessage(admin: ReturnType<typeof createClient>, channel: Channel): Promise<FeedMessage> {
  let lines: string[]
  ...
  if (channel.channel_type === 'weather') {
    ...
    if (!response.ok) throw new Error(`Weather feed failed: ${response.status}`)
    ...
    lines = ['NOCHI LOYALTY', `${metric?.attached_orders || 0} VISITS`, `${metric?.attach_rate || 0}% ATTACH RATE`]
  } else if (channel.channel_type === 'custom' || channel.channel_type === 'special') {
    const value = String(channel.config?.message || channel.name)
    lines = value.split('\n').slice(0, 3)
  } else {
    const quotes = [
      ['GOOD COFFEE', 'GOOD MOOD', 'NOCH'],
      ['STAY CURIOUS', 'DRINK MATCHA', 'AT NOCH'],
      ['SMALL SIPS', 'BIG IDEAS', 'NOCH'],
    ]
    lines = pick(quotes)
  }

  const characters = framed(lines, COLOR.GREEN, COLOR.VIOLET)
  return {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const channel = forceType ? null : await getDueChannel(admin, new Date())
    const settings = await getAutomationSettings(admin)
    const message = channel
      ? await buildChannelMessage(admin, channel)
      : await chooseMessage(settings, forceType)
    ...
    if (channel) {
      ...
      await admin
        ...
        .update({
          ...
          next_run_at: new Date(now.getTime() + channel.cadence_minutes * 60000).toISOString(),

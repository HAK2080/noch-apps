-- Stock receiving is an operational action available to every authenticated
-- employee. Branch assignment remains the authorization boundary; the profile
-- active flag is intentionally not used because login access is managed
-- separately and legacy employee profiles may have a stale flag.
...
begin
  ...
  if p_actor_profile_id is not null and not exists (
    ...
    raise exception 'Stock reporter is not assigned to this branch';
  end if;

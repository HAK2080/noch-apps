-- POS stamp awards now roll over at stamp_goal and write full history.
-- 20260718180500 credited current_stamps/total_stamps only; customers never
-- reached their free-drink reward, no loyalty_stamps rows were written, and
-- tiers were never recomputed. award_loyalty_stamps mirrors award_loyalty_stamp
-- (20260604040000) minus the streak/visit bookkeeping (handled by
-- record_pos_customer_visit). No backfill here: 20260718180500 already
-- credited historical orders.
...
begin
  if p_count is null or p_count <= 0 then
    return jsonb_build_object('success', true, 'stamps_awarded', 0);
  end if;

  select * into v_customer from loyalty_customers where id = p_customer_id for update;
  if not found then
  ...
  v_new_stamps := coalesce(v_customer.current_stamps, 0);
  v_new_total := coalesce(v_customer.total_stamps, 0);

  for v_i in 1..p_count loop
    v_new_stamps := v_new_stamps + 1;
    v_new_total := v_new_total + 1;

    insert into loyalty_stamps (customer_id, awarded_by, stamp_number, cycle_number, notes)
    values (
      p_customer_id, null, v_new_stamps,
...
revoke execute on function public.award_loyalty_stamps(uuid, int) from public, anon;
grant execute on function public.award_loyalty_stamps(uuid, int) to authenticated, service_role;

-- Same body as 20260718180500; only the stamp-crediting block now routes
-- through award_loyalty_stamps and can never fail the order.
...
begin
  ...
  if p_offline_order_number is not null and length(p_offline_order_number) > 0 then
    ...
    select upper(coalesce(
      nullif(regexp_replace(string_agg(left(w, 1), '' order by ord) filter (where w ~ '^[A-Za-z]'), '[^A-Za-z]', '', 'g'), ''),
      'POS'
    )) into v_branch_code
    from (
      select w, ord
      from unnest(string_to_array((select name from pos_branches where id = p_branch_id), ' ')) with ordinality as words(w, ord)
    ) sub;
  ...
  insert into pos_orders (
    ...
    presto_collected, customer_name, customer_phone
  ) values (
    ...
    v_presto_collected, v_customer_name, v_customer_phone
  )
  ...
  loop
    ...
    if coalesce((v_item->>'track_inventory')::boolean, false) and (v_item->>'product_id') is not null then
      update pos_products
  ...
  if v_stamps_awarded > 0 and p_loyalty_customer_id is not null then
    begin
      perform public.award_loyalty_stamps(p_loyalty_customer_id, v_stamps_awarded);

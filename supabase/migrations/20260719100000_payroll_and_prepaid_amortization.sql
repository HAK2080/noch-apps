-- Payroll labor components and prepaid expense amortization in finance_pnl.
--
-- 1. labor_adjustments: owner-managed overtime / bonus / deduction entries that
--    feed the P&L labor leg. branch_id null = consolidated only.
-- 2. expenses.coverage_months / coverage_start: prepaid expenses covering more
--    than one month are amortized day-by-day across their coverage window
--    instead of hitting the P&L in full on expense_date.
-- 3. finance_pnl: the labor leg is split into hourly + salary + adjustments,
--    and the canonical-expenses opex leg amortizes prepaid rows.
--
-- ONE-PAY-PATH RULE: staff with monthly_salary > 0 are costed via the salary
-- path and excluded from the hourly leg; staff without a salary are costed via
-- shift_labor_cost. Branch P&L allocates each salaried person's prorated
-- amount by their share of shift hours at that branch in the period; a
-- salaried person with no logged hours in the period contributes 0 to branch
-- P&Ls but their full amount to the consolidated P&L.

-- ── 1. Labor adjustments ─────────────────────────────────────────────────────
create table public.labor_adjustments (
  id uuid primary key default gen_random_uuid(),
  profile_id uuid not null references profiles(id),
  branch_id uuid references pos_branches(id),  -- null = consolidated only
  kind text not null check (kind in ('overtime', 'bonus', 'deduction')),
...
alter table public.labor_adjustments enable row level security;

create policy "labor_adjustments_read" on public.labor_adjustments
  for select to authenticated
  using (true);

create policy "labor_adjustments_owner_insert" on public.labor_adjustments
  for insert to authenticated
  with check (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'owner'));

create policy "labor_adjustments_owner_delete" on public.labor_adjustments
  for delete to authenticated
  using (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'owner'));

grant select, insert, delete on public.labor_adjustments to authenticated;

-- ── 2. Prepaid coverage on canonical expenses ────────────────────────────────
alter table public.expenses
  add column if not exists coverage_months int not null default 1 check (coverage_months between 1 and 60),
  add column if not exists coverage_start date;

-- ── 3. finance_pnl: hourly + salary + adjustments labor, amortized opex ──────
...
as $$
  ...
  -- Salary leg: each salaried profile's monthly_salary prorated day-by-day
  -- over every calendar month overlapping [p_from, p_to]. Branch P&L takes the
  -- person's share of shift hours at that branch; no hours logged anywhere in
  -- the period means 0 for branches, full amount for the consolidated view.
  labor_salary as (
    select coalesce(sum(prorated.amount_lyd * prorated.branch_share), 0) as salary_lyd
    from (
      select
        pr.id,
        sum(
      ...
      group by pr.id, pr.monthly_salary, sh.branch_hours, sh.total_hours
    ) prorated
  ),
  -- Adjustments leg: overtime and bonus add, deductions subtract.

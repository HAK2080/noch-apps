-- Guest Order Submission RPC Function
-- Single function to submit complete order with items
...
BEGIN
  ...
  -- NOTE: loyalty_customers uses full_name (not name), no branch_id column
  BEGIN
...
GRANT EXECUTE ON FUNCTION submit_guest_order(uuid, text, text, text, jsonb) TO anon, authenticated, service_role;

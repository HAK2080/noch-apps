...
-- Sold-item costs are immutable from this rollout onward; older items keep
-- the legacy current-cost fallback and are explicitly counted as unverified.
...
as $$
  ...
  cogs as (
    select coalesce(sum(coalesce(item.unit_cost_lyd_at_sale, product.cost_lyd, 0) * item.quantity), 0) as cogs_lyd
  ...
  modifier_cogs as (
    select coalesce(sum(coalesce(selected_modifier.cost_delta_lyd_at_sale, modifier.cost_delta_lyd, 0) * item.quantity), 0)
      as modifier_cogs_lyd
    from pos_order_item_modifiers selected_modifier
    left join pos_modifiers modifier on modifier.id = selected_modifier.modifier_id
  ...
  payroll_estimate_salary_rows as (
    ...
    where selected.run_id is null
      ...
      and coalesce(profile.monthly_salary, profile.monthly_salary_lyd, 0) > 0
  ),
  salary_rows as (
  ...
  cost_quality as (
    select count(distinct product.id) filter (
      where product.id is not null
        and (coalesce(item.unit_cost_lyd_at_sale, product.cost_lyd) is null
          or coalesce(item.unit_cost_lyd_at_sale, product.cost_lyd) <= 0)
    ) as missing_product_cost_count,
    count(*) filter (where item.product_id is not null and item.cost_snapshot_at is null)
      as unverified_historical_cost_items
    from pos_orders orders
    join pos_order_items item on item.order_id = orders.id
    left join pos_products product on product.id = item.product_id
    cross join bounds
  ...
  modifier_cost_quality as (
    select count(*) filter (where selected_modifier.cost_snapshot_at is null)
      as unverified_historical_modifier_cost_items
    from pos_orders orders
    join pos_order_items item on item.order_id = orders.id
    join pos_order_item_modifiers selected_modifier on selected_modifier.order_item_id = item.id
    cross join bounds
    where (p_branch_id is null or orders.branch_id = p_branch_id)
      and orders.status = 'completed'
      and orders.created_at >= bounds.from_utc
  ...
  select jsonb_build_object(
    ...
    'data_quality', jsonb_build_object(
      ...
      'unverified_historical_cost_items', (select unverified_historical_cost_items from cost_quality),
      'unverified_historical_modifier_cost_items',
        (select unverified_historical_modifier_cost_items from modifier_cost_quality),
...
-- The historical product matrix uses the same 05:00 window and captured base
-- costs as P&L. A weighted average is needed when cost or price changed during
-- the selected period; modifier COGS remains a separate P&L component.
...
language sql stable security definer set search_path=public as $$
  with bounds as (
    ...
    select
      ...
      sum(coalesce(item.unit_cost_lyd_at_sale, product.cost_lyd, 0) * item.quantity)::numeric as cogs,
      bool_and(coalesce(item.unit_cost_lyd_at_sale, product.cost_lyd, 0) > 0) as has_cost
    from public.pos_order_items item
    join public.pos_orders orders on orders.id=item.order_id
    left join public.pos_products product on product.id=item.product_id
    cross join bounds
    where (p_branch_id is null or orders.branch_id=p_branch_id)
      and orders.status='completed'
      and orders.created_at>=bounds.from_utc
  ...
  select
    ...
    null::uuid,
    sold.has_cost,
    sold.revenue / nullif(sold.units_sold,0) as unit_price,
    sold.cogs / nullif(sold.units_sold,0) as unit_cost,
    (sold.revenue-sold.cogs) / nullif(sold.units_sold,0) as contribution_margin,
    case when sold.revenue>0 then (sold.revenue-sold.cogs)/sold.revenue else 0 end
      as contribution_margin_ratio,
    sold.units_sold,
    sold.revenue,
    sold.revenue-sold.cogs as total_contribution
  from sold
...
language sql stable security definer set search_path=public as $$
  select
    item.product_id,
    coalesce(item.product_name,'(deleted)'),
    sum(item.quantity)::numeric,
    sum(item.total)::numeric,
    sum(item.quantity * coalesce(item.unit_cost_lyd_at_sale, product.cost_lyd, 0))::numeric,
    (sum(item.total)
      - sum(item.quantity * coalesce(item.unit_cost_lyd_at_sale, product.cost_lyd, 0)))::numeric
  from public.pos_order_items item
  join public.pos_orders orders on orders.id=item.order_id
  left join public.pos_products product on product.id=item.product_id
  where orders.branch_id=p_branch_id and orders.status='completed'
    and orders.created_at>=p_from and orders.created_at<p_to

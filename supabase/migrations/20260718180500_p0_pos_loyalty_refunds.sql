-- P0 POS correctness and privilege fixes.
...
-- Counter orders award the configured order stamps to the real loyalty columns.
...
begin
  ...
  if v_stamps_awarded > 0 and p_loyalty_customer_id is not null then
    update loyalty_customers
      set current_stamps = coalesce(current_stamps, 0) + v_stamps_awarded,
          total_stamps = coalesce(total_stamps, 0) + v_stamps_awarded,
          updated_at = now()
...
-- Restore the order-derived stamps which the old RPC silently discarded.
do $$
declare
  v_goal int := greatest(coalesce((select stamp_goal from loyalty_settings limit 1), 9), 1);
begin
  update loyalty_customers c
     set total_stamps = coalesce(c.total_stamps, 0) + x.awarded,
         current_stamps = mod(coalesce(c.current_stamps, 0) + x.awarded, v_goal),
         updated_at = now()
    from (
      select loyalty_customer_id as customer_id,
             sum(coalesce(loyalty_stamps_awarded, 0))::int as awarded
        from pos_orders
       where loyalty_customer_id is not null
       group by loyalty_customer_id
    ) x
   where c.id = x.customer_id and x.awarded > 0;
end $$;

-- Make every future partial refund visible to finance and GL.
...
-- Reconcile existing rows, including rows whose previous value was non-zero.
update pos_orders o
   set refunded_amount_lyd = coalesce((
     select sum(coalesce(oi.refunded_qty, 0) * oi.unit_price)
       from pos_order_items oi
      where oi.order_id = o.id and coalesce(oi.refunded_qty, 0) > 0
   ), 0);

-- Voids only restock tracked inventory and only the quantity not already refunded.

-- Extend create_pos_order to accept and persist customer_name. The
-- column already exists on pos_orders (used by submit_guest_order for
-- online orders) but the counter-sale RPC silently dropped the value.
--
-- We DROP the old signature and recreate — adding a parameter is a
-- signature change that CREATE OR REPLACE can't do in-place.

DROP FUNCTION IF EXISTS public.create_pos_order(
  uuid, uuid, uuid, uuid, numeric, numeric, numeric, numeric, text,
  numeric, numeric, numeric, uuid, timestamptz, text, jsonb
);
...
begin
  -- Normalise customer_name: trim then null-out empties
  ...
  insert into pos_orders (
    ...
    presto_collected,
    customer_name           -- NEW: persists counter-sale name
  ) values (
...
-- Re-grant execute (DROP removed the grants)

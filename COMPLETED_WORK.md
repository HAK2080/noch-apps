...
- **Commit / deployment**: Committed on `perf/pre-sized-product-images`. Not merged or deployed. Existing product images still have no stored derivatives, so they fall back to the master and continue to work, but the quota only reaches zero once they are re-uploaded or backfilled.

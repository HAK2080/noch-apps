...
- **Agent**: Codex
- **Status**: Design-only scope complete; proposed program and implementation still require approval.
- **Files**: `docs/research/noch-loyalty-rebuild/05-detailed-program-design.md`, `06-design-at-a-glance.md`; updated navigation/status in `01-executive-summary.md`, `04-points-protection-and-project-goal.md`; `COMPLETED_WORK.md`.
- **Description**: Completed program rules, six customer journeys, 13 screen specifications, bilingual working copy, marketing roles/approval controls, bounded AI assistance, transaction/recovery contracts, preservation rules and acceptance gates. Explicitly recommends future customer-selected redemption without presenting it as implemented or approved. Existing points and inherited reward terms remain protected.
- **Verification**: Read current guidelines and research/protection baseline; clean starting worktree at `41bd987`; inspected Git comparison confirms mapped loyalty/marketing modules, PaymentModal and main loyalty migrations unchanged from `b2ed42d`. Documentation links and whitespace checked. No new live test, backup, reconciliation or migration claimed.
- **Commit / deployment**: None by this task. No application code or customer data changed; design preparation is not implementation authorization.
...
- **Commit / deployment**: Merged to `main` (92eb8d5) and deployed to apps.noch.cloud. The migration was applied to production by the owner through the Supabase SQL editor in three chunks on 2026-09-11, after the single-transaction file failed to paste in full; the applied objects were then verified present (table, `pos_settings.block_duplicate_tabs`, both triggers) and the PostgREST schema cache reloaded. Both switches are live and default to off. Note: production DDL was applied by hand rather than by running the migration file, so the file itself has never been executed end to end against a real database — treat a fresh environment as unproven.
...
## 2026-09-11 — Pre-Sized Product Images (Storage Transformation Quota Fix)

- **Agent**: Claude
- **Status**: Implemented and verified locally. Not deployed; existing images still need a backfill before the quota reaches zero.
- **Files**: `apps/pos/src/lib/product-images.js`, `apps/pos/src/modules/pos/lib/product-image-processing.js`, `apps/pos/src/modules/pos/lib/pos-supabase.js`, `apps/pos/src/modules/pos/pages/POSProducts.jsx`, `apps/pos/src/pages/ProductCatalog.jsx`, `apps/pos/src/modules/pos/components/ProductGrid.jsx`, `apps/pos/src/pages/storefront/Menu.jsx`; tests `product-image-stored-variants.test.mjs` (new), `product-image-optimization.test.mjs`, `product-catalog-image-optimization.test.mjs`, `product-image-generation.test.mjs`, `customer-menu-performance.test.mjs`.
- **Description**: The organisation exceeded its Supabase Pro quota for Storage Image Transformations (190 of 100 included), which is billed per origin image, while egress and storage stayed below 1%. Every product image was served through `/storage/v1/render/image/public/` with width/height/quality parameters, so each new product photo consumed quota permanently. Images are now rendered once at upload into three stored WebP sizes — full 1200x1500 (<=300 KB), card 720x900 (<=120 KB) and thumb 160x200 (<=12 KB) — in a single decode pass, uploaded alongside the master under a suffixed name, and addressed directly by `buildStoredProductImageUrl`. All four display sites (POS product grid, POS product form, menu category icon, menu card/detail) request a stored size; no source file references the transformation endpoint any more. `ProductCatalog.handleImageUpload` previously uploaded the raw camera file unoptimized and now renders the stored set like every other path. Menu cards move from a 360 px transform to a 720 px stored file, which is sharper on 2x/3x phone screens at a similar byte size; the category icon drops from 64 px to a 160 px stored file. Legacy images without derivatives fall back to the master URL through the existing `onError` handlers on every call site.
- **Verification**: `node tests/product-image-stored-variants.test.mjs` — 7/7 passed, covering variant URL construction, degradation for empty/external/extension-less/unknown-variant inputs, a dot-in-folder path case, master addressability after upload, and a guard asserting no source file references `render/image`. Full `apps/pos/tests/*.test.mjs` sweep run by exit code: only `system-acceptance-privacy` and `workforce-control` fail, both pre-existing on `origin/main` and reading only App/profiles/finance/workforce files that this change does not touch. `npm run build` succeeded; ESLint clean on all changed files (0 errors; remaining warnings are pre-existing `exhaustive-deps`). Not verified in a browser or against production: actual quota decline, real rendering quality, and derivative upload behaviour remain unproven until deployed.
- **Commit / deployment**: Merged to `main` (41bd987) and deployed to apps.noch.cloud on 2026-09-11. Open item: existing product images still have no stored derivatives, so they fall back to the master URL and keep working, but the transformation quota only reaches zero once every image is re-saved or backfilled. Not yet confirmed against the Supabase usage page, which refreshes hourly.
...
- **Verification**: Payroll regression test passed after first reproducing the old overflow failure; classic-skin tests, targeted ESLint, POS production build, and `git diff --check` passed. Authenticated production checks found 10 cards with 11 inputs each, no document/card horizontal overflow at 1960 px, 1274 px, or 384 px, and no console errors. The wide layout renders all 11 fields in one row; narrower layouts wrap them within the employee card.
- **Migration**: None.
- **Commit**: `fec37e2` (`fix(payroll): replace scrolling table with responsive cards`)
- **Deployment**: GitHub Actions run `30641389210` deployed successfully to `apps.noch.cloud`; authenticated live smoke checks passed afterward.
...
- **Verification**: ESLint passed, production build passed, focused tests passed, migrations compiled in rollback transactions, and production database objects plus both migration-ledger entries were verified live. GitHub deployment run #215 succeeded. A live owner walkthrough confirmed the page, branch selection, and count picker; it exposes 68 tracked products and 79 ingredients without creating a count.
...
- **Files**:
  ...
  - `apps/pos/tests/korea-edition-menu.test.mjs`
- **Description**: The customer menu serves a 140 KB desktop Korea/Japan composition and a separate 48 KB mobile header derived from the supplied reference. The campaign expands beyond the standard 700 px menu column to a prominent 960 px desktop stage, where four larger live cards align with the artwork placeholders; smaller screens use a taller header crop and swipeable cards up to 286 px wide. The menu also uses a 12 KB logo, smaller contained product derivatives, optimized category icons, first-section-only image priority, a 12-hour stale-while-refresh menu cache, and narrow Supabase field selection.
...
- **Agent**: Codex
- **Status**: Complete & Live
- **Backup**: GitHub branch `backup/pre-ai-product-image-20260816` preserves the previous live commit `bd51126`.
- **Files**:
...
- **Deployment**: Supabase Edge Function `generate-product-image` was deployed to project `kxqjasdvoohiexedtfqw`, including compatibility with the project's existing `Openai_API_KEY` secret; CORS returned 200 and an unauthenticated POST returned 401 as expected. GitHub Actions runs `31941218981` and corrective run `31941633155` deployed `main` successfully to `apps.noch.cloud`. Production serves the corrected global editor in `ProductCatalog-V9Ex_SjB.js`; no-cache checks returned 200 and confirmed the live Main category selector, additional-category controls, shared filtering, and `secondary_category_ids` write path.
...
- **Agent**: Codex.
- **Files**: `apps/pos/src/modules/finance/CEOOverview.jsx`, `modules/finance/lib/ceo-money.js`, `modules/pos/lib/global-stock.js`, `modules/pos/components/GlobalStockControl.jsx`, POS/product/menu integration, owner navigation, expense payment-date input, two migrations, and focused tests. Source map: `docs/finance-ceo-and-stock-map.md`.
- **Behavior**: Owner landing at `/overview` shows money in, money out and net movement; defaults to Libya month-to-date with custom dates; separate invoice totals and prorated payroll estimate; editable dated cash/bank observations with an independent discrepancy check against the preceding observation and recorded movements. First observation establishes a baseline; edits preserve history and never post a balancing journal.
- **Stock control**: Owner global switch in Products and POS Settings defaults off. When enabled, rejects missing setup, zero/negative and insufficient stock; checks authoritative branch quantities and actual coffee-bean consumption, shades blocked POS images, validates stale carts/server requests and pending online completion, and prevents offline success when protection is enabled. Every new checkout must obtain the current policy online. Cost-only recipes do not qualify as stock setup.
- **Payroll data**: Recorded the owner's August total once as a shared Staff expense, then confirmed cash payment on August 31. Corrected the existing payment button's default date using the audited settlement function and verified a balanced posted journal. Existing August payroll draft preserved for reconciliation; no additional cash payment was made.
...
## 2026-09-12 — Record paid scanned invoices using entry dates
- **Agent**: Codex.
- **Authorization**: Owner confirmed every scanned invoice is paid, instructed cash assumption, then explicitly required the system entry date rather than the paper date.
- **Data**: Settled 18 approved scanned expenses totalling 9,385.50 LYD via audited `mark_expense_paid`, using Libya-local submitted_at dates. Guarded expected row count/amount/funding and retained the assumption in payment notes/reference. Existing paid and rejected scans preserved.
- **Verification**: Rolled-back production rehearsal, then committed execution; zero journal mismatches. Independent query confirms 18 paid cash records, zero date/status mismatches, zero remaining approved scans. September money in 24,921.50, money out 9,393.50 and net movement 15,528 LYD. Audit details in `docs/finance-ceo-audit-2026-09-12.md`.
- **Deployment**: Data correction is live. Reporting code `425f94e` is already deployed by successful run `34674741454`. Month-end expected-payments card remains a recommendation pending rent/regular-bill amounts and due dates; no incomplete forecast presented as a total.

## 2026-09-12 — New scanned receipts default to paid cash
- **Agent**: Codex.
- **Files**: Receipt Snap PWA; expense-snap and telegram-webhook Edge Functions; migration `20260912130000_scanned_receipt_payment_entry_date.sql`; focused database/Edge/browser tests and fixtures.
- **Behavior**: Photo scans default to paid cash and go directly to branch allocation; unpaid/card overrides remain available before saving, including changing an earlier selection. Unreadable amounts retain the default after manual amount entry. Typed expenses keep explicit payment selection. Existing expense approval policy is preserved; approved scanned payments use Libya-local system entry dates rather than extracted paper dates.
- **Verification**: 20 focused checks passed (11 PostgreSQL approval tests, five executable scanner/Telegram tests, four existing declaration tests), targeted ESLint and production build passed, plus a phone browser flow from photo to default cash, manual unpaid override and save. Screenshot reviewed. No production test invoice was posted.
- **Commit / deployment**: `16a416e` pushed; GitHub reports successful app deployment for the commit. Applied the date migration; live function definition confirms entry-date policy. Deployed expense-snap and telegram-webhook via Supabase editor, both showing fresh deployment timestamps; existing shared stock-command module preserved. Unrelated research and prior work-log changes preserved.

## 2026-09-12 — September payroll estimate from August paid payroll

...
const optimizedBannerUrl = new URL('../public/assets/korea-japan-banner.webp', import.meta.url)
...
test('Korean and Japanese drinks use the supplied banner without replacing live products', async () => {
  ...
  assert.match(menu, /\/assets\/korea-japan-banner\.webp/)
...
test('reference-matched section renders four live cards across and adapts on mobile', async () => {
  const [css, bannerStats, optimizedBannerStats] = await Promise.all([
    readFile(menuCssUrl, 'utf8'),
    stat(bannerUrl),
    stat(optimizedBannerUrl),
  ])
  ...
  assert.ok(optimizedBannerStats.size < 150 * 1024, `Optimized banner should stay below 150 KB, got ${optimizedBannerStats.size}`)
})

...
test('product form provides one-click generation and reuses the no-crop optimizer before save', async () => {
  ...
  assert.match(source, /await optimizeProductImageSet\(generatedFile\)/)
  assert.match(source, /stageOptimizedImage\(optimized\)/)
  assert.match(source, /await uploadProductImage\(savedProduct\.id,/)
  assert.match(source, /pendingVariants \? \{ file: pendingFile, variants: pendingVariants \} : pendingFile\)/)
})

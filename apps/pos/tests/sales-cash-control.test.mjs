...
test('critical sales and cash-control journeys use the authoritative interfaces', async () => {
  ...
  assert.match(sessions, /getSalesControlSummary/)
  assert.match(sessions, /normalizeSalesControl/)
  assert.match(sessions, /normalizeShiftControl/)
  assert.match(sessions, /setLoading\(true\)/)
  assert.match(sessions, /copy\.businessRange/)
  assert.match(sessions, /title: 'Sales control'/)
  assert.match(sessions, /totalSales: 'Net sales'/)
  assert.match(sessions, /money\(periodControl\.net_sales\)/)
  assert.match(sessions, /periodControl\.period_cash_movement/)
  ...
  assert.match(sales, /shifts: 'Sales control'/)
  assert.match(sales, /<TrendingUp size=\{14\} \/> \{copy\.shifts\}/)
...
test('owner sales, refund, shift, and closeout controls have explicit Arabic copy', async () => {
  ...
  assert.match(sessions, /رقابة المبيعات/)
  assert.match(sessions, /صافي المبيعات/)

...
test('cashier QR decodes; separate customer page verifies, links and displays settlement response', async ({ page, context }, info) => {
  ...
  await expect(customer.getByText('لديك 1 مكافأة جاهزة')).toBeVisible()
  await expect(customer.getByText('تم إكمال 2 من 3')).toHaveCount(0)
  ...
  await customer.getByRole('button', { name: 'ليس الآن', exact: true }).click()

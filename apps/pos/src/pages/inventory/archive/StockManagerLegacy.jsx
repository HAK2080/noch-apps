...
export default function StockManager() {
  ...
  const mergedItems = ingredients.map(ing => {
    ...
    return {
      ...
      theoretical_qty: theoretical[ing.id]?.theoretical_qty ?? null,
      consumed_since_count: theoretical[ing.id]?.consumed_since_count ?? null,
      recipe_usage_available: theoretical[ing.id]?.recipe_usage_status === 'available',
        ...
        {item.track_type !== 'equipment' && (
          <div className={`rounded-lg px-3 py-2 text-xs border ${item.count_is_stale ? 'border-yellow-500/30 bg-yellow-500/5' : 'border-noch-border bg-noch-dark'}`}>
            <div className="flex justify-between">
              <span className="text-noch-muted">Estimated now</span>
              <b className={item.recipe_usage_available ? 'text-white' : 'text-red-300'}>
                {item.recipe_usage_available
                  ? `${smartQty(item.theoretical_qty, item.unit).value} ${smartQty(item.theoretical_qty, item.unit).unit}`
                  : 'Unavailable — recipe missing'}
              </b>
            </div>
            <div className="flex justify-between mt-1">
              <span className="text-noch-muted">Explicit recipe usage since count</span>
              <span className={item.recipe_usage_available ? 'text-yellow-300' : 'text-red-300'}>
                {item.recipe_usage_available
                  ? `-${smartQty(item.consumed_since_count, item.unit).value} ${smartQty(item.consumed_since_count, item.unit).unit}`
                  : 'Not calculated'}
              </span>
            </div>
          </div>
        ...
        {!isArchived && (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {item.min_threshold > 0
              && (item.recipe_usage_available ? item.theoretical_qty : item.qty_available) <= item.min_threshold && (
              <button onClick={() => navigate(`/inventory/procurement?ingredient=${item.id}&qty=${Math.max(item.min_threshold - (item.recipe_usage_available ? item.theoretical_qty : item.qty_available), 0)}`)} className="border border-yellow-500/30 text-yellow-300 rounded-lg px-3 py-1.5 text-xs font-medium hover:bg-yellow-500/10">
                Create order
              </button>
            )}
            {isOwner && (

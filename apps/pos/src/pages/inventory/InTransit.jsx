...
export default function InTransit() {
  ...
  useEffect(() => {
    let active = true
    Promise.all([listLocations(), listInTransit()])
      .then(([locationRows, transitRows]) => {
        if (!active) return
      ...
      .catch(error => {
        if (!active) return
        toast.error(error.message || (arabic ? 'تعذر تحميل المخزون قيد النقل' : 'Failed to load in-transit stock'))
        setLoading(false)
      })
    return () => { active = false }
  }, [arabic])
  ...
  async function load() {
    ...
    try {
      ...
      toast.error(err.message || copy('Failed to load in-transit stock', 'تعذر تحميل المخزون قيد النقل'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const locationName = id => {
    ...
    return (arabic ? location?.name_ar || location?.name : location?.name)
      || copy('Unknown branch', 'فرع غير معروف')
  }
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="flex-1">
            <h1 className="text-white font-bold text-xl flex items-center gap-2">
              ...
              {copy('In Transit', 'قيد النقل')}
            </h1>
            <p className="text-noch-muted text-sm">{copy('Shipped from warehouse, not yet received', 'شُحن من المستودع ولم يُستلم بعد')}</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="bg-noch-card border border-noch-border rounded-xl p-10 text-center">
            ...
            <p className="text-noch-muted text-sm">{copy('Nothing in transit right now', 'لا يوجد مخزون قيد النقل حاليًا')}</p>
          </div>
        ) : (
              ...
              return (
                <div key={locId} className="bg-noch-card border border-noch-border rounded-xl overflow-hidden">
                  <div className="px-4 py-3 border-b border-noch-border flex items-center justify-between">
                    ...
                    <span className="text-amber-400 text-xs font-bold">{total} {copy('units inbound', 'وحدة قادمة')}</span>
                  </div>
                  <div className="divide-y divide-noch-border/50">
                    {items.map((r, i) => (
                      <div key={`${r.product_id}-${i}`} className="flex items-center justify-between px-4 py-2.5">
                        <p className="text-white text-sm truncate">{r.product_name || copy('Unknown product', 'منتج غير معروف')}</p>

...
function CatIcon({ name, imageUrl, size = 16 }) {
  ...
  return <span style={{ fontSize: size }}>{catEmoji(name)}</span>
}
...
function catEmoji(name = '') {
  ...
  if (/pancake|بان كيك|فطائر/.test(n)) return '🥞'
        ...
        return (
          <div key={p.id} className={`addon-item${soldOut ? ' sold-out' : ''}`}>
            ...
            {!soldOut && (qty === 0 ? (
              ...
              <div className="qty-ctrl qty-ctrl-xs">
                <button className="qty-btn" onClick={() => onRemove(p.id)}>−</button>
                <span>{qty}</span>
                <button className="qty-btn" onClick={() => onAdd(p.id)}>+</button>
              </div>
...
export default function Menu() {
  ...
  useEffect(() => {
    ...
    if (branch.lat == null || branch.lng == null) { setGps({ status: 'unavailable', lat: null, lng: null, distance: null }); return }
  ...
  const finalTotal = couponApplied ? Math.max(0, cartTotal - couponApplied.discount_amount) : cartTotal
  ...
  async function applyCoupon() {
    ...
    setCouponError(''); setCouponChecking(true)
    try {
      const { data, error } = await supabase.rpc('apply_coupon', {
        p_code: couponCode.trim(), p_branch_id: branchId, p_order_total: cartTotal,
      })
      if (error) throw error
      if (data?.valid) { setCouponApplied(data); setCouponError('') }
      else { setCouponApplied(null); setCouponError(data?.message || 'Invalid code') }
    } catch { setCouponError('Could not check code. Try again.') }
  ...
  return (
    <div className="menu-root">
      ...
      <header className="menu-header">
        <div className="menu-header-inner">
          <div className="menu-header-left">
            <a href="https://noch.cloud/" className="menu-back" aria-label="Back">
              {lang === 'ar' ? '→' : '←'}
            </a>
            <img src={nochLogo} alt="Noch" className="menu-logo" />
            <div>
      ...
      <div className="cat-strip" dir={lang === 'ar' ? 'rtl' : 'ltr'} ref={catStripRef}>
        {hasFeatured && (
          <button
            className={`best-sellers-chip${showFeatured ? ' active' : ''}`}
            onClick={() => { setShowFeatured(f => !f); setActiveCat('all') }}
          >
        ...
        <button
          id="pill-all"
          className={`cat-pill${activeCat === 'all' && !showFeatured ? ' active' : ''}`}
          onClick={() => { setActiveCat('all'); setShowFeatured(false) }}
        >
          🍽️&nbsp;{t('All', 'الكل')}
        </button>
        {categories.map(c => {
          ...
          return (
            <button
              key={c.id}
              id={`pill-${c.id}`}
              className={`cat-pill${activeCat === c.id && !showFeatured ? ' active' : ''}`}
              onClick={() => { setActiveCat(c.id); setShowFeatured(false) }}
            >
            ...
            {!orderResult && (
              <>
                ...
                <div className="sheet-header-row">
                  ...
                  <button className="sheet-cancel-btn" onClick={() => !submitting && setShowCheckout(false)} disabled={submitting}>
                    {t('✕ Cancel', '✕ إلغاء')}
                  </button>
                </div>
                ...
                <div className="coupon-section">
                  <div className="coupon-row">
                    <input
                      ...
                      value={couponCode}
                      onChange={e => { setCouponCode(e.target.value.toUpperCase()); setCouponError(''); if (couponApplied) setCouponApplied(null) }}
                ...
                <div className="sheet-form">
                  ...
                  <input className="sheet-input" value={phone} onChange={e => setPhone(e.target.value)}
                    placeholder={t('Phone (e.g. 0912345678)', 'رقم الهاتف')} type="tel" disabled={submitting} />

                  {gps.status === 'checking' && <div className="gps-row">📍 {t('Checking your location…', 'جارٍ التحقق من موقعك…')}</div>}
                  {gps.status === 'outside' && (
                    <div className="gps-row warn">
                      {t(`You need to be at ${branch.name} to place an order.`, `يجب أن تكون في ${branch.name} لتقديم الطلب.`)}
                      {gps.distance != null && <> ({Math.round(gps.distance)} m {t('away', 'بُعداً')})</>}
                    </div>
                  )}
                  {gps.status === 'denied' && <div className="gps-row warn">{t('Please enable location access, then reopen checkout.', 'يرجى تفعيل الموقع ثم إعادة فتح صفحة الدفع.')}</div>}
                  {gps.status === 'unavailable' && <div className="gps-row warn">{t('Location check unavailable. Please order with a staff member.', 'تحقق الموقع غير متاح. تواصل مع الكاشير مباشرة.')}</div>}

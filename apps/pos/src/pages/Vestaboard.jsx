    ...
    <Layout>
      <div className="mx-auto max-w-7xl space-y-5 p-4 md:p-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          {isOwner && (
            <button
              onClick={() => navigate('/vestaboard/channels')}
              className="btn-primary ms-auto flex items-center gap-2"
            >
              <Monitor size={14} /> Noch Channels
            </button>
          )}
        </div>
          ...
          <div className="card">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-white">Customer greetings</h2>
              </div>
              <button
                type="button"
                role="switch"
              ...
              >
                <span className={`relative h-5 w-9 rounded-full transition-colors ${
                  greetingsEnabled ? 'bg-noch-green' : 'bg-noch-border'
                }`}>
                  <span className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${
                    greetingsEnabled ? 'translate-x-[18px]' : 'translate-x-0.5'
                  }`} />
                </span>
                <span className="min-w-7 text-left text-xs font-semibold">
                  {greetingSettingLoading ? '…' : greetingsEnabled ? 'On' : 'Off'}
                </span>
              </button>
            </div>
            <p className="text-sm text-noch-muted">
              {greetingsEnabled
                ? 'POS fires a colorful greeting after every named order.'
                : 'Order-triggered greetings are paused.'}
            </p>
